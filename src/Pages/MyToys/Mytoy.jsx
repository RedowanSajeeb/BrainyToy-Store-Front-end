...
const Mytoy = () => {
  ...
  // --------------------------
  const  aSCENDING = () =>{
    fetch(`https://brainy-toy-store-server-side.vercel.app/brainyemailascending?email=${user?.email}`)
    .then(res => res.json())
    .then( data => setUsertoy(data) )
  }
  ...
  const dESCENDING = () => {
    fetch(`https://brainy-toy-store-server-side.vercel.app/brainyemdescending?email=${user?.email}`)
      .then((res) => res.json())
      .then((data) => setUsertoy(data));
  };
  // --------------------------
    ...
    <div className="max-w-main mx-auto md:ms-14 md:mr-14">
      {user && (
        <h1 className="md:text-3xl text-2xl text-center mt-5 md:mt-16">
          Hey{" "}
          <span className="text-orange-600 text-2xl">{user?.displayName}</span>{" "}
          Your Added Toy Itms
        </h1>
      )}
      {/* ----------------------------------------------------------- */}
